...
\section{Abstract}
Photon transport through turbid media has typically been modeled through diffusion or telegraph equations. These models describe behavior of the average, or typical, photon with remarkable accuracy, however, we show here that they fail to capture the Extreme First Passage Times (EFPTs) of photon transport. By sending ultra-short bursts of photons through a scattering medium and timing the arrival of the first passage photon, we measure the distribution of these EFPTs of photons in a random environment. Our measured EFPTs differ from those predicted by both the diffusion approximation and telegraph equation. Instead, we observe the EFPT as the time expected for light to travel through an index-averaged medium. These results reveal flaws in both models and invite a re-examining of their underlying assumptions.

\section{Motivation}
In Chapter \ref{ch3_extras}, we explored the possibility of measuring extreme first passage times (EFPTs) in colloid diffusion, with the goal that the statistics of these distributions can reveal information about the shared environment through connections made in numerical work~\cite{hass_anomalous_2023,hass_first-passage_2024,hass_extreme_2024}. During my graduate work, we also sought to measure extreme first passage times in another, vastly different system: photons travelling through scattering media, which appear to also behave ``diffusively''. Using an ultrafast pulsed laser, we send very narrow bursts of photons into a narrow, polished-interior tube filled with silica nanosphere suspensions, making a quasi-1D scattering medium. With a single photon avalanche diode (SPAD) and time-correlated single photon counter (TCSPC), we time the first photon to pass through to the other end, down to 2ps precision. Through power attenuation optics and built-in wavelength tuning, we can control the number of photons $N$ in the diffusive system. By adjusting the scattering properties of the medium, particularly in varying the concentration of silica nanospheres in the suspensions, we alter the properties of the underlying environment. Utilizing a high repetition rate from the laser and a fast photon counting setup allows for collection of a large number of EFPTs in a very short amount of time. This provides a quick method for building EFPT distributions, which can then be analyzed to determine the extreme value statistics for this diffusive system.

We designed the experimental system such that different elements can be changed without affecting the path length. By using two rotating polarizers, we can attenuate the power while keeping the polarization consistent across concentrations. We flush solutions through the tube using hoses and gravity-driven flow to enable concentration changes without interfering with the setup. An XYZ translational stage keeps the SPAD mounted in place while allowing for precise repositioning without drastically increasing measurement uncertainty, particularly when neutral density filters need to be added at low scatterer concentrations to avoid oversaturating the detector. These conditions allow for measurements of EFPT with extreme (5-10ps) precision and ensure the experiment's repeatability.

This chapter reports on the relationship between scatterer concentration and the peak of the EFPT distribution, as well as the failure of the underlying random walk model in capturing the extreme behavior of photons travelling through scattering media. Additional work would be required to study the variance of these distributions to make connections to the motivating numerical work~\cite{hass_anomalous_2023,hass_extreme_2024,hass_first-passage_2024}, but through the experimental work described here, we have the potential to discover new insights into the effects of the shared environment on scattering photons.
...
In this chapter, we review the history of models for the first passage of photons, then describe an experimental setup to measure photon EFPT. We describe the diffusion approximation and telegraph equation approaches to modeling photon transport, and compare EFPT predictions from these models to experimental measurements. We demonstrate that both models fail to predict the EFPT.
...
\section{Conclusion}
The diffusion and telegraph approximations fail to accurately describe EFPT photons, either violating causality (diffusion) or predicting unrealistically large EFPT differences (diffusion and telegraph). In order to provide accurate predictions, the telegraph equation  would require either unphysically large photon counts of $N \gtrapprox 10^{16}$, far exceeding the laser output to achieve wavelength-independent results, or an anisotropy factor ($\langle \cos {\theta} \rangle \geq 0.4$) that contradicts the isotropic-scattering behavior of Rayleigh scattering. While these models are effective at predicting the behavior of the average photon in scattering media and can be used to infer bulk properties, they fail to describe extreme photons. These results suggest that assumptions made in the underlying random walk approach are incorrect. Near-field effects present at high scatterer number density and the breakdown of self-similarity in photon transport could provide a route toward explaining why EFPTs follow the index-averaged predictions long after increased scattering should have increased delays, as these effects can produce photon trajectories that favor forward-scattering~\cite{pattelli_role_2018,pattelli_experimental_2022,pini_non-self-similar_2024,rezvani_naraghi_near-field_2015}. Alternatively, a model based on random walks which incorporate correlated or even quantum mechanical behavior may produce more accurate predictions and could better describe the underlying physical process. Such a model would unlock the potential for photon EFPT statistics to uncover critical insights about the diffusive environment, with broad applications across many areas of science~\cite{redner_guide_2001,weiss_applications_2002,godec_first_2016,grebenkov_molecular_2021,polizzi_mean_2016,barney_first-passage-time_2017,zsurkis_first_2024,lawley_distribution_2020,lawley_slowest_2023,schuss_redundancy_2019,meerson_mortality_2015,chun_heterogeneous_2023}.
